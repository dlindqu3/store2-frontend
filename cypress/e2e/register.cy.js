...
describe('tests layout and functionality of Register page', () => {
  it('contains an input field with correct data-testid', () => {
    ...
    cy.get("[data-testid='register-username-field']").should('exist')
  })

  it('does not contain a paragraph with error text at initial page load', () => {
    cy.visit("http://localhost:3000/register")
    cy.get("[data-testid='register-error']").should('not.exist')
  })

  it('contains a paragraph with correct error text if password too short', () => {
    cy.visit("http://localhost:3000/register")
    cy.get("[data-testid='register-username-field']").type('batman75')
    cy.get("[data-testid='register-email-field']").type('batman75@gmail.com')
    cy.get("[data-testid='register-password-field']").type('gotham')
    cy.get("[data-testid='register-submit-button']").click()
    cy.get("[data-testid='register-error']").contains('**Please enter a password with at least 8 characters.')
  })
  ...
  it('should redirect to login route after successful registration', () => {
    ...
    cy.get("[data-testid='register-username-field']").type(randomStrings[0])
    cy.get("[data-testid='register-email-field']").type(`${randomStrings[1]}@gmail.com`)
    cy.get("[data-testid='register-password-field']").type(`gotham@${randomStrings[2]}`)
    cy.get("[data-testid='register-submit-button']").click() 
